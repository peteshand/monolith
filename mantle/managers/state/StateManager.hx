...
import mantle.managers.conditions.ConditionItem;
import mantle.managers.conditions.ConditionManager;
 ...
 * @author P.J.Shand
 */
@:access(mantle.managers.conditions.ConditionManager)
class StateManager 
{
	...
	private function OnSceneChange():Void 
	{
		...
		for (key in stateObjects.keys())
		{
			...
			if (item.wildcard) {
				...
				if (sceneModelURI.indexOf(item.uri) == 0) {
	...
	public function addURI(uri:String, condition:ConditionItem=null):StateManager 
	{
	...
	public function removeURI(uri:String):StateManager 
	{
	...
	public function maskURI(uri:String):StateManager 
	{
	...
	public function ignore(uri:String):StateManager 
	{
	...
	public function addCondition(notifier:Notifier<Dynamic>, value:Dynamic, operation:String="=="):StateManager 
	{
		conditionManager.add(notifier, value, operation);
		return this;
	}
	
	public function removeCondition(notifier:Notifier<Dynamic>):StateManager 
	{
		conditionManager.remove(notifier);
		return this;
	}
	
	public static function fromURI(uri:String):StateManager 
	{
		return new StateManager().addURI(uri);
	}
	
	public function attachTransition(transition:Transition):Void 
	{
		if (transition == null) {
			warn("transition not set");
			return;
		}
	...
	public function clone():StateManager
	{
		var _clone:StateManager = new StateManager();
		for (key in stateObjects.keys())
		{
			stateObjects.set(key, stateObjects[key]);
		}
		for (key in conditionManager.conditionObjects.keys())
		{
			_clone.conditionManager.conditionObjects.set(key, conditionManager.conditionObjects[key]);
		}
		return _clone;
	}
